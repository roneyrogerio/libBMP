...
/*   Updated: 2020/07/02 10:53:07 by rde-oliv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lbmp	*lbmp_load(char *pathname)
{
	...
	if ((lbmp = (t_lbmp *)calloc(1, sizeof(t_lbmp))) == NULL)
	...
	if (lbmp->iheader.bpp != 24 && lbmp->iheader.bpp != 32
			&& lbmp_destroy(lbmp))
	...
	lbmp->fheader.size = (lbmp->fheader.size - lbmp->fheader.offset) + 54;
